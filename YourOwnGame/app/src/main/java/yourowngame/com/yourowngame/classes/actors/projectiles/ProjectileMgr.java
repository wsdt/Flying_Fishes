...
public class ProjectileMgr {
    ...
    public static void runDefaultConfiguration(@NonNull Activity activity) {
        ...
        if (!alreadyExecuted) {
            for (int i = 0; i < 5; i++) {
                Projectile p = new Projectile_Iron(activity, Projectile.DEFAULT_SPEED_X, Projectile.DEFAULT_SPEED_Y);
                p.initialize();
    ...
    public static void updateProjectiles() {
        ...
        for (Iterator<Projectile> it = getShotProjectiles().iterator(); it.hasNext(); ) {
            ...
            if (p.isNotVisible()) { //should also work, bc. in resetPos() we set the bullet by default out of screen!
